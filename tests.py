...
class TreeTestCase(unittest.TestCase):
    def tree_add(self):
        ...
        self.assertEqual(node.children[0], 5)
        self.assertEqual(node.children[0], 6)

        traversed = []
        for n in node.walk():
            traversed.append(n.value)

        self.assertEqual(traversed[0], 5)
        self.assertEqual(traversed[1], 6)
        self.assertEqual(traversed[2], 3)
...
class DumbQueryBuilderTestCase(unittest.TestCase):
    ...
    def test_and_or_builder(self):
        ...
        self.assertEqual(where.clause(), expected)

    def test_and_or_with_empty_q(self):
